...
function main(lines)
    ...
    bin_tree = zeros(Int, ((file_length ÷ 2), file_width))
    bin_tree[1, file_width÷2 + 1] = 1
    ...
    for idx in collect(3:2:file_length-1)
        ...
        for jdx in collect(1:file_width)
            ...
            # Fill integer tree
            if (bin_tree[kdx-1, jdx] > 0) && lines[idx][jdx] == '^'
                bin_tree[kdx, jdx-1] += bin_tree[kdx-1, jdx]
                bin_tree[kdx, jdx+1] += bin_tree[kdx-1, jdx]
            else  # if (bin_tree[kdx-1, jdx] > 0) && lines[idx][jdx] == '.'
                bin_tree[kdx, jdx] += bin_tree[kdx-1, jdx]
            end
        end
    ...
    result2 = sum(bin_tree, dims=2)[end]
    # display(bin_tree)
...
lines = readlines(f)
final_answer1, final_answer2 = main(lines)
close(f)


println("Final answer1=$(final_answer1), final answer2=$(final_answer2)")
print(("="^10) * "DONE" * ("="^10)*"\n")