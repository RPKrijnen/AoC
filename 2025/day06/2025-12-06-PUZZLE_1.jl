...
function main(df)
    ...
    for idx in collect(1:n_cols)
        ...
        for jdx in collect(1:n_rows-1)
            push!(temp_res, parse(Int, df[jdx, idx]))
        end
        # println(temp_res)
        if df[end, idx] == "+"
            final_res += sum(temp_res)
        else  # df[end, idx] == "*"
            final_res += prod(temp_res)
        end
    end
...
# data = readlines(f)
# println(data)
df = CSV.read("2025\\day06\\puzzle_input.csv", DataFrame; delim=" ", ignorerepeated=true, types=String, validate=false)
